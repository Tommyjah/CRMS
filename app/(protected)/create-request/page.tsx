...
export default async function CreateRequestPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-50/50 dark:bg-zinc-950 flex items-center justify-center px-4">
        <div className="text-center p-8 max-w-md rounded-xl border border-rose-200/50 dark:border-rose-800/50 bg-white dark:bg-zinc-900 shadow-sm">
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-zinc-100 mb-4">Not Authenticated</h1>
          <p className="text-slate-600 dark:text-zinc-400 mb-6">You must be signed in to create change requests.</p>
          <Link href="/login" className="rounded-lg bg-teal-600 px-4 py-2 text-sm font-medium text-white hover:bg-teal-700 transition-colors inline-block">
            Sign In
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-slate-50/50 dark:bg-zinc-950 flex items-center justify-center px-4">
        <div className="text-center p-8 max-w-md rounded-xl border border-rose-200/50 dark:border-rose-800/50 bg-white dark:bg-zinc-900 shadow-sm">
          ...
          <p className="text-slate-600 dark:text-zinc-400 mb-6">Approvers cannot create new requests. Switch to REQUESTER role in Settings if needed.</p>
  ...
  return <CreateRequestForm userProfile={userProfile} />
}
